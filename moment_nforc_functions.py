...
def get_modeshapes(odb, node_set_name_1, node_set_name_2, save=False, name='test', folder='test'):
    ...
    for i in range(number_of_modes):
        ...
        # Field output
        displacement_y = frame.fieldOutputs['NFORC6']
        displacement_z = frame.fieldOutputs['NFORC5']
        rotation_x = frame.fieldOutputs['NFORC4']
        ...
        for j in range(0, n_sensor_nodes * 2, 2):
            ...
            if j < len(node_set_1.nodes[0]) * 2:
                ...
                # Field output data
                Y = round(node_displacement_y_1.values[j].data, 10)
                Z = round(node_displacement_z_1.values[j].data, 10)
                T = round(node_rotation_x_1.values[j].data, 10)
                ...
                # Field output data
                Y = round(node_displacement_y_2.values[j - len(node_set_1.nodes[0]) * 2].data, 10)
                Z = round(node_displacement_z_2.values[j - len(node_set_1.nodes[0]) * 2].data, 10)
                T = round(node_rotation_x_2.values[j - len(node_set_1.nodes[0]) * 2].data, 10)
            ...
            phi_matrix[0, i, 358] = round(node_displacement_y_1.values[715].data, 10)
            phi_matrix[1, i, 358] = round(node_displacement_z_1.values[715].data, 10)
            phi_matrix[2, i, 358] = round(node_rotation_x_1.values[715].data, 10)

            phi_matrix[0, i, 608] = round(node_displacement_y_2.values[499].data, 10)
            phi_matrix[1, i, 608] = round(node_displacement_z_2.values[499].data, 10)
            phi_matrix[2, i, 608] = round(node_rotation_x_2.values[499].data, 10)
